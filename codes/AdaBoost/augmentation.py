...
class Resize(torch.nn.Module):
    ...
    def __init__(self, size, image_interpolation=Image.BILINEAR, mask_interpolation=Image.NEAREST):
        super().__init__()
        if not isinstance(size, (int, Sequence)): raise TypeError(
            'Size should be int or sequence. Got {}'.format(type(size)))
        if isinstance(size, Sequence) and len(size) not in (1, 2): raise ValueError(
...
class Pad(torch.nn.Module):
    ...
    def __init__(self, padding, fill=0, padding_mode="constant"):
        ...
        if padding_mode not in ['constant', 'edge', 'reflect', 'symmetric']: raise ValueError(
            'Padding mode should be either constant, edge, reflect or symmetric')
        if isinstance(padding, Sequence) and len(padding) not in [1, 2, 4]: raise ValueError(
...
class RandomCrop(torch.nn.Module):
    ...
    def get_params(image, output_size):
        ...
        if (h + 1 < th) or (w + 1 < tw): raise ValueError(
...
class RandomPerspective(torch.nn.Module):
    ...
    def forward(self, image, mask):
        if torch.rand(1) < self.p:
            ...
            return F.perspective(image, startpoints, endpoints, self.interpolation, self.fill), F.perspective(mask,
                                                                                                              startpoints,
                                                                                                              endpoints,
                                                                                                              self.interpolation,
                                                                                                              self.fill)
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def forward(self, image, mask):
        ...
        return F.resized_crop(image, i, j, h, w, self.size, self.image_interpolation), F.resized_crop(mask, i, j, h, w,
                                                                                                      self.size,
...
class ColorJitter(torch.nn.Module):
    ...
    def _check_input(self, value, name, center=1, bound=(0, float('inf')), clip_first_on_zero=True):
        if isinstance(value, numbers.Number):
            ...
            if not bound[0] <= value[0] <= value[1] <= bound[1]: raise ValueError(
...
class RandomRotation(torch.nn.Module):
    ...
    def forward(self, image, mask):
        ...
        return F.rotate(image, angle, self.resample, self.expand, self.center, self.fill), F.rotate(mask, angle,
                                                                                                    self.resample,
                                                                                                    self.expand,
                                                                                                    self.center,
                                                                                                    self.fill)
...
class RandomAffine(torch.nn.Module):
    ...
    def forward(self, image, mask):
        ...
        return F.affine(image, *ret, resample=self.resample, fillcolor=self.fillcolor), F.affine(mask, *ret,
...
class GaussianBlur(torch.nn.Module):
    ...
    def __init__(self, kernel_size, sigma=(0.1, 2.0)):
        ...
        if isinstance(sigma, numbers.Number):
            ...
            if not 0. < sigma[0] <= sigma[1]: raise ValueError(
